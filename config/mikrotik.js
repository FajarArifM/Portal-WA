...
async function connectToMikrotik() {
    try {
        ...
        const host = getSetting('mikrotik_host', '192.166.20.1');
        const port = parseInt(getSetting('mikrotik_port', '8729'));
        const user = getSetting('mikrotik_user', 'genieacs');
        const password = getSetting('mikrotik_password', 'Adminjoss1');
...
async function getResourceInfo() {
    ...
    const interfaceName = process.env.MAIN_INTERFACE || '00.SFP+ 10G 1 UPSTREAMS';
