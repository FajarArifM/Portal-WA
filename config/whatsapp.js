...
function decryptAdminNumber(encryptedNumber) {
    try {
        ...
        const key = '3ANGEL.NET_SECRET_KEY_2025';
        let result = '';
...
function formatWithHeaderFooter(message) {
    try {
        ...
        const header = getSetting('company_header', '3ANGEL.NET BOT MANAGEMENT ISP');
        ...
        return `🏢 *3ANGEL.NET BOT MANAGEMENT ISP*\n\n${message}\n\nInternet Tanpa Batas`;
    }
}
...
function generateWatermark() {
    ...
    const secretKey = process.env.SECRET_KEY || '3ANGEL.NET';
...
async function connectToWhatsApp() {
    try {
        ...
        sock = makeWASocket({
            auth: state,
            logger,
            browser: ['3ANGEL.NET', 'Chrome', '1.0.0'],
        ...
        sock.ev.on('connection.update', (update) => {
            ...
            if (connection === 'open') {
                ...
                try {
                    // Pesan notifikasi
                    const notificationMessage = `📱 *BOT WHATSAPP 3ANGEL.NET NETWORK*\n\n` +
                    ...
                    `💰 *Dukungan Pengembang:*\n` +
                    `• E-WALLET: 082140501327\n` +
                    `• BRI: 3506 01 003507 50 7 a.n PT MULTI NETWORK SOLUTIONS\n\n` +
...
▸ *setheader [teks_header_baru]* — Ganti header pesan bot
   Contoh: setheader 3ANGEL.NET HOTSPOT
▸ *setfooter [teks_footer_baru]* — Ganti footer pesan bot
   Contoh: setfooter Powered by 3ANGEL.NET Digital Network
